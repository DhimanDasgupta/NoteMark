    ...
    spec {
      ...
      inState<RegistrationState> {
        ...
        on<UserNameFiledLostFocus> { action ->
          val modifiedState =
            if (action.userName.isUsernameValid()) {
              snapshot.copy(
                userNameExplanation = null,
                userNameError = null,
              )
        ...
        on<UserNameEntered> { action ->
          val modifiedState =
            snapshot.copy(
              ...
              userNameExplanation =
                if (action.userName.isEmpty()) "Use between 3 and 20 characters for your username"
                else null,
            )
        ...
        on<PasswordEntered> { action ->
          val modifiedState =
            snapshot.copy(
              ...
              repeatPasswordError =
                if (action.password != snapshot.repeatPassword) "Passwords do not match" else null,
            )
          mutate { modifiedState.validateNonEmptyInputs() }
        }
        on<RepeatPasswordEntered> { action ->
          ...
          modifiedState =
            modifiedState.copy(
              repeatPasswordError =
                if (action.repeatPassword != snapshot.password) "Passwords do not match" else null
            )
          mutate { modifiedState.validateNonEmptyInputs() }
        }
...
private fun RegistrationState.validateNonEmptyInputs(): RegistrationState =
  copy(
    registrationEnabled =
      userName.isNotEmpty() &&
        email.isNotEmpty() &&
...
private fun RegistrationState.validateInputs(): RegistrationState {
  var updatedRegistrationState =
    copy(
      userNameError =
        if (!userName.isUsernameValid()) "Username must be at least 3 characters" else null
    )
  updatedRegistrationState =
    updatedRegistrationState.copy(
      emailError = if (!email.isValidEmail()) "Invalid email provided" else null
    )
  updatedRegistrationState =
    updatedRegistrationState.copy(
      passwordError =
  ...
  updatedRegistrationState =
    updatedRegistrationState.copy(
      repeatPasswordError =
        if (!repeatPassword.isValidPassword())
          "Password must be at least 8 characters and include a number or symbol"
        else null
    )
