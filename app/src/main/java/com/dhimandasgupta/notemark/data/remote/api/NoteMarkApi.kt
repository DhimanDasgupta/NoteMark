      ...
      when (response.status) {
        ...
        // Consider handling other specific statuses like BadRequest, Unauthorized, etc.
          ...
          Result.failure(
            exception =
              ApiGenericException(
                message = "Registration failed with status: ${response.status.value}"
              )
          )
      }
    ...
    return try {
      ...
      when (response.status) {
        ...
        // Consider handling other specific statuses like BadRequest, Unauthorized, etc.
          ...
          Result.failure(
            exception =
              ApiGenericException(message = "Logout failed with status: ${response.status.value}")
          )
      }
    } catch (e: AuthenticationException) {
      Result.failure(
      ...
      when (response.status) {
        HttpStatusCode.OK -> {
          ...
          Result.failure(
            exception =
              ApiGenericException(
                message = "Create Note failed with status: ${response.status.value}"
              )
          )
      }
    } catch (e: AuthenticationException) {
