...
/**
 * A utility class that manages a collection of shared Flows, ensuring proper
 * lifecycle management and resource sharing for concurrent consumers in a
 * coroutine scope.
 *
 ...
 * @property replayExpiration The duration after which the sharing stops
 *     if no collectors are present. Defaults to [Duration.INFINITE].
 * @property stopTimeout The duration to wait before stopping the sharing
 *     after all consumers have unsubscribed. Defaults to [Duration.ZERO].
 * @property replay The number of most recent items to replay to new collectors.
 * @property builder A function that defines how to build a Flow when a
 *     requested key is not already present in the collection.
 */
