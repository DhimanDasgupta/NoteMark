 ...
 *
 * @param activeState The minimum Lifecycle.State required for the click to be processed. Defaults
 *   to RESUME.
...
/**
 * Aligns the composable to the safe drawing insets, effectively avoiding system UI elements like
 ...
 * and then places it by offsetting its y-position downwards by the top safe drawing inset. The
 * width and height of the composable remain as measured. This is useful for positioning content
 * below the status bar.
 *
 * It is important to note that this only accounts for the `top` inset. For more complex scenarios
 * involving other insets (e.g., `bottom`, `start`, `end`), a more comprehensive window insets
...
fun Modifier.alignToSafeDrawing(): Modifier {
  return layout { measurable, constraints ->
    if (constraints.hasBoundedWidth && constraints.hasBoundedHeight) {
      ...
      layout(width, height) {
        val bottom =
          WindowInsetsRulers.SafeDrawing.current.bottom.current(defaultValue = 0f).roundToInt() -
            height
        val right =
          WindowInsetsRulers.SafeDrawing.current.right.current(defaultValue = 0f).roundToInt()
        val left =
          WindowInsetsRulers.SafeDrawing.current.left.current(defaultValue = 0f).roundToInt()
        measurable
          .measure(
            constraints =
              Constraints.fixed(
