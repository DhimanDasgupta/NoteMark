...
/** A simple username validation. Allows alphanumeric chars, underscore, and hyphen. Length 3-20. */
 ...
 *
 * This function processes a name string and returns a formatted version based on the following
 * rules:
 ...
 *     - If the single name part has 2 or fewer characters, it's returned in uppercase (e.g., "Al"
 *       -> "AL").
 *     - If the single name part has more than 2 characters, the first two characters are returned
 *       in uppercase (e.g., "John" -> "JO").
 * - In the unlikely event that no name parts are found after trimming and splitting (which should
 *   be handled by the initial empty check), it returns an empty string as a fallback.
 *
 * Examples:
 * - "John Doe" -> "JD"
 * - "SingleName" -> "SI"
...
fun String.formatUserName(): String {
  val nameParts =
    split(SPACE_REGEX).filter { string ->
      string.isNotEmpty()
