 ...
 *
 * @param producer A suspending function that produces values to emit into the flow. This function
 *   will be called repeatedly to generate new values.
 * @return A flow that emits values produced by the producer function indefinitely.
 */
fun <T> infiniteFlowOf(producer: suspend () -> T) =
  flow<T> {
    while (true) {
      emit(producer())
    }
...
/**
 * Collects elements from the upstream flow and invokes [action] with a list of elements as soon as
 * [batchSize] elements are accumulated. When the flow terminates (normal completion, cancellation,
 * or exception), [action] is invoked with the remaining collected elements.
 *
...
/**
 * Emits value every [period], if there is one. Emits immediately, if time between two consecutively
 * emitted values is greater or equals [period]
 */
...
fun <T> Flow<T>.retryBackoff(
  ...
  beforeRetry: suspend (cause: Throwable, currentAttempts: Int, totalAttempts: Int) -> Unit =
    { _, _, _ ->
    },
  retriesExhausted: suspend (cause: Throwable) -> Unit = {},
  retryCondition: suspend (cause: Throwable, currentAttempts: Int, totalAttempts: Int) -> Boolean =
    { _, _, _ ->
      true
    },
): Flow<T> {
  ...
  return flow {
    ...
    this@retryBackoff.retryWhen { cause, totalAttempts ->
        isRetrying = true
...
private fun calculateBackoffDelay(
  ...
  val baseDelay = (minDelayMillis * backoffFactor.pow(currentAttempts)).coerceAtMost(maxDelayMillis)
