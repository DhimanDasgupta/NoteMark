...
class FakeFailureNoteRepository : NoteMarkRepository {
  ...
  override fun getNotesFromOffSetWithLimit(
    limit: Long,
    offset: Long,
  ): Flow<List<NoteEntity>> = flowOf(value = emptyList())

  override fun getAllNotes(): Flow<List<NoteEntity>> = flowOf(value = emptyList())

  override suspend fun getRemoteNotes(
    page: Int,
    size: Int,
  ): Result<NoteResponse> = Result.failure(Exception("Some exception"))
