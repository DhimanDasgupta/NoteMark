...
class FakeSuccessfulNoteMarkApi : NoteMarkApi {
  ...
  override suspend fun login(request: LoginRequest): Result<Unit> = Result.success(value = Unit)

  override suspend fun getNotes(page: Int, size: Int): Result<NoteResponse> =
    Result.success(value = noteResponse)

  override suspend fun logout(request: RefreshRequest): Result<Unit> = Result.success(value = Unit)
