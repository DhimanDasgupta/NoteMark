...
class RetryBackoffFlowTest {
  ...
  fun should_retry_single_failing_call() = runTest {
    ...
    flow {
      ...
      .testBackoffRetry(
        ...
        expectSuccessAfterRetry = listOf(SuccessAfterRetryCall(attempts = 1, time = 1000)),
        expectRetriesExhaustedCalls = listOf(),
        expectBeforeRetryCalls = listOf(BeforeRetryCall(error, 0, 0, 0)),
      )
  }

  @Test
  fun should_retry_multiple_times_with_growing_backoff() = runTest {
    failingNTimesFlow(10)
      .testBackoffRetry(
        testScope = this,
        minDelay = 1.seconds,
        maxAttempts = 11,
        jitterFactor = 0.0,
        expectResult = Result.success(listOf("ABC")),
        expectRetriesExhaustedCalls =
          listOf(), // Should not call retriesExhausted when maxAttempts is not reached
        expectSuccessAfterRetry =
          listOf(
            SuccessAfterRetryCall(
              attempts = 10,
              time = 512000 + 256000 + 128000 + 64000 + 32000 + 16000 + 8000 + 4000 + 2000 + 1000,
            )
          ),
        expectBeforeRetryCalls =
          listOf(
            ...
            BeforeRetryCall(
              TestException("Error7"),
              6,
              6,
              32000 + 16000 + 8000 + 4000 + 2000 + 1000,
            ),
            BeforeRetryCall(
              TestException("Error8"),
              7,
              7,
      ...
      .testBackoffRetry(
        ...
        expectBeforeRetryCalls =
          listOf(
            ...
            BeforeRetryCall(
              TestException("Call10"),
              1,
              7,
              1000 + 4000 + 2000 + 1000 + 4000 + 2000 + 1000,
            ),
            BeforeRetryCall(
              TestException("Call11"),
              2,
              8,
              2000 + 1000 + 4000 + 2000 + 1000 + 4000 + 2000 + 1000,
            ),
          ),
      )
  }
      ...
      .testBackoffRetry(
        ...
        expectSuccessAfterRetry =
          listOf(
            ...
            SuccessAfterRetryCall(
              attempts = 9,
              time = 256000 + 128000 + 64000 + 32000 + 16000 + 8000 + 4000 + 2000 + 1000,
            ),
          ),
        expectBeforeRetryCalls =
          listOf(
  ...
  fun should_use_backoff_factor() = runTest {
    failingNTimesFlow(10)
      .testBackoffRetry(
        ...
        expectRetriesExhaustedCalls = listOf(RetryExhaustedCall(TestException("Error6"), 121000)),
  ...
  fun should_stop_backoff_growing_once_max_reached() = runTest {
    failingNTimesFlow(10)
      .testBackoffRetry(
        ...
        expectRetriesExhaustedCalls =
          listOf(), // Should not call retriesExhausted when maxAttempts is not reached
        expectSuccessAfterRetry = listOf(SuccessAfterRetryCall(10, 7 * 5000 + 4000 + 2000 + 1000)),
        expectBeforeRetryCalls =
          listOf(
            BeforeRetryCall(TestException("Error1"), 0, 0, 0),
            BeforeRetryCall(TestException("Error2"), 1, 1, 1000),
            BeforeRetryCall(TestException("Error3"), 2, 2, 2000 + 1000),
            BeforeRetryCall(TestException("Error4"), 3, 3, 4000 + 2000 + 1000),
            // Delay is random due to jitter, but once it reached maxDelay it should be around this
            // value
  ...
  fun should_never_have_delay_smaller_than_min_and_bigger_than_max() = runTest {
    ...
    assertTrue(
      "All delay times should be between 2750 and 4500 ms, those outside the limit are ${delayTimes.filter { it !in 2750..4500 }}"
    ) {
  ...
  fun should_retry_until_max_attempts_reached() = runTest {
    failingNTimesFlow(10)
      .testBackoffRetry(
        ...
        expectSuccessAfterRetry = listOf(),
        expectRetriesExhaustedCalls = listOf(RetryExhaustedCall(TestException("Error5"), 24000)),
        expectBeforeRetryCalls =
          listOf(
            BeforeRetryCall(TestException("Error1"), 0, 0, 0),
