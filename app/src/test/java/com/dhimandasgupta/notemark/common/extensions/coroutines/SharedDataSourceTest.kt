...
class SharedDataSourceTest {
  ...
  private val infiniteEverySecondProducerFlow =
    generateSequence(0) { it + 1 }.asFlow().onEach { delay(1000) }

  @Test
  fun should_use_builder_to_create_a_flow() = runTest {
    val pool =
      SharedDataSource(scope = backgroundScope) { key: String ->
        flowOf("1$key", "2$key", "3$key").onEach { delay(1000) }
      }
  ...
  fun should_reuse_the_same_flow_for_the_same_key() = runTest {
    var createdFlows = 0
    val pool =
      SharedDataSource(scope = backgroundScope) { _: String ->
        createdFlows++
  ...
  fun should_reply_last_value() = runTest {
    ...
    pool.get("A").onEach { valueProduced = it }.launchIn(backgroundScope)
  ...
  fun should_keep_lifecycle_order() = runTest {
    ...
    val pool2 =
      SharedDataSource(backgroundScope) { key: String ->
        infiniteEverySecondProducerFlow
