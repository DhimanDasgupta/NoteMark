    ...
    withContext(Dispatchers.Default) {
      dataFlow(10)
        ...
        .onEach {
          ...
          if (delta < period.inWholeMilliseconds) assertEquals(period.inWholeMilliseconds, delta)
...
class MapAsyncConcurrencyTest {
  ...
  fun should_behave_like_a_regular_map_for_a_list_and_a_set() = runTest {
    ...
    assertEquals(
      list.map(charTransformation1),
      list.mapAsync(concurrency = anyConcurrency, charTransformation1),
    )
    val charTransformation2 = { c: Char -> c.code }
    assertEquals(
      list.map(charTransformation2),
      list.mapAsync(concurrency = anyConcurrency, charTransformation2),
    )
    val charTransformation3 = { c: Char -> c.uppercaseChar() }
    assertEquals(
      list.map(charTransformation3),
      list.mapAsync(concurrency = anyConcurrency, charTransformation3),
    )

    val set = (1..10).toSet()
    val intTransformation1 = { i: Int -> i * i }
    assertEquals(
      set.map(intTransformation1),
      set.mapAsync(concurrency = anyConcurrency, intTransformation1),
    )
    val intTransformation2 = { i: Int -> "A$i" }
    assertEquals(
      set.map(intTransformation2),
      set.mapAsync(concurrency = anyConcurrency, intTransformation2),
    )
    val intTransformation3 = { i: Int -> i.toChar() }
    assertEquals(
      set.map(intTransformation3),
      set.mapAsync(concurrency = anyConcurrency, intTransformation3),
    )
  }
  ...
  @Test
  fun should_limit_concurrency_for_different_delays() =
    testFor(
...
class MapAsyncTest {
  ...
  @Test
  fun should_map_async_and_keep_elements_order() = runTest {
    val transforms: List<suspend () -> String> =
      listOf(
        {
          delay(3000)
          "A"
...
class OnEachBatchTest {
  ...
  fun `should invoke action for full batches`() = runTest {
    ...
    val result = flowOf(1, 2, 3, 4, 5, 6).onEachBatch(3) { batches.add(it) }.toList()
  ...
  fun `should invoke action for remaining elements at the end`() = runTest {
    val batches = mutableListOf<List<Int>>()
    val result = flowOf(1, 2, 3, 4, 5).onEachBatch(3) { batches.add(it) }.toList()
  ...
  fun `should handle empty flow`() = runTest {
    val batches = mutableListOf<List<Int>>()
    val result = emptyFlow<Int>().onEachBatch(3) { batches.add(it) }.toList()
  ...
  fun `should flush remaining elements when downstream fails`() = runTest {
    ...
    assertEquals(
      listOf(listOf(1, 2)),
      batches,
      "Should flush remaining elements even if downstream fails",
    )
  }
...
class RaceOfTest {
  ...
  fun should_wait_for_the_fastest_for_big_number() = runTest {
    val racers =
      List<suspend CoroutineScope.() -> Long>(1000) { i ->
          {
  ...
  fun should_cancel_slower() = runTest {
    ...
    val result =
      raceOf(
        ...
        {
          slowerJob = currentCoroutineContext().job
          delay(2000)
          "B"
        },
      )
  ...
  fun should_cancel_when_parent_cancelled() = runTest {
    ...
    val job = launch {
      raceOf(
        { delay(1000) },
        {
          innerJob = currentCoroutineContext().job
          delay(2000)
        },
      )
    }
  ...
  fun should_propagate_context() = runTest {
    ...
    withContext(coroutineName1) {
      raceOf(
        { delay(1000) },
        {
          innerCtx = currentCoroutineContext()
          delay(2000)
        },
      )
    }
